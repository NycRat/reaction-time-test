...
pub fn HomePage(cx: Scope) -> impl IntoView {
    ...
    let (results, set_results) = create_signal(cx, Vec::<TestResult>::new());
    ...
    //     move || {
    //         set_running(true);
    //         set_start_time(chrono::Utc::now().timestamp_millis());
    //     },
    //     std::time::Duration::from_millis(rand::thread_rng().gen_range(1500, 5000)),
    // );
